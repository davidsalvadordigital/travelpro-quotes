...
function CustomTooltip({ active, payload, label }: { active?: boolean; payload?: { name: string; value: number | string; color: string }[]; label?: string }) {
    ...
    return (
        <div className="rounded-2xl border border-glass-border bg-glass backdrop-blur-xl px-5 py-4 shadow-2xl text-[11px] font-bold uppercase tracking-widest animate-in fade-in zoom-in-95 duration-200">
            <p className="text-muted-foreground mb-3 pb-2 border-b border-glass-border">{label}</p>
    ...
    return (
        <ResponsiveContainer width="100%" height={280}>
            <BarChart data={data} barSize={20} barGap={6}>
                ...
                />
                <Bar dataKey="cotizaciones" name="Total" fill="oklch(0.65 0.18 210)" radius={[4, 4, 0, 0]} />
                <Bar dataKey="ganadas" name="Éxito" fill="oklch(0.62 0.25 350)" radius={[4, 4, 0, 0]} />
                <Bar dataKey="perdidas" name="Pérdidas" fill="oklch(0.25 0.05 240 / 0.15)" radius={[4, 4, 0, 0]} />
            </BarChart>
        </ResponsiveContainer>
    );
}
...
export function LeadPieChart({ data }: { data: LeadStatusData[] }) {
    ...
    const EXECUTIVE_COLORS = [
        "oklch(0.62 0.25 350)", // Rosa (Principal)
        "oklch(0.65 0.18 210)", // Cian (Secundario)
        "oklch(0.55 0.12 320)", // Púrpura
        "oklch(0.25 0.05 240)", // Navy
    ];
            ...
            <PieChart>
                ...
                >
                    {data.map((entry, index) => (
                        <Cell key={entry.name} fill={EXECUTIVE_COLORS[index % EXECUTIVE_COLORS.length]} />
                    ))}
                </Pie>
            ...
            <AreaChart data={data}>
                <defs>
                    <linearGradient id="colorCoti" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="oklch(0.65 0.15 235)" stopOpacity={0.15}/>
                        <stop offset="95%" stopColor="oklch(0.65 0.15 235)" stopOpacity={0}/>
                    </linearGradient>
                    <linearGradient id="colorGana" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="oklch(0.60 0.22 350)" stopOpacity={0.15}/>
                        <stop offset="95%" stopColor="oklch(0.60 0.22 350)" stopOpacity={0}/>
                    </linearGradient>
                </defs>
                <CartesianGrid vertical={false} strokeDasharray="0" stroke="currentColor" strokeOpacity={0.03} />
                <XAxis 
